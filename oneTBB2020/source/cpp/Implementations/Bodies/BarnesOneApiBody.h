...
#include "../../DataStructures/BarnesOneApiDataStructures.h"
...
class BarnesOneApiBody
        : public IOneApiBody {
private:
    static void ComputeForce(OctTreeLeafNode *body, OctTreeLeafNode *bodies, OctTreeInternalNode *tree,
...
public:
    ...
    BarnesOneApiBody(Params p) : firsttime{true}, gpu_queue(cl::sycl::queue(cl::sycl::gpu_selector{})) {
    ...
    void OperatorCPU(int begin, int end) {
        for (int i = begin; i < end; i++) {
            BarnesOneApiBody::ComputeForce(&bodies[i], bodies, tree, step, epssq, dthf);
        }
    }
    ...
    int NewNode(const float px, const float py, const float pz) {
        ...
        int i;
        OctTreeInternalNode &ptr = tree[num_cells];
 ...
 * **************************************************************************/
    ...
    void initialize_tree() {
        ...
        tree = (OctTreeInternalNode *) cl::sycl::malloc_shared(
                sizeof(OctTreeInternalNode) * nbodies, gpu_queue);
        num_cells = 0;
        num_cmax = nbodies;
    }
    ...
    inline void ReadInput(char *filename) {
        ...
        if (bodies == NULL) {
            ...
            bodies = (OctTreeLeafNode *) cl::sycl::malloc_shared(sizeof(OctTreeLeafNode) * nbodies, gpu_queue);
            bodies2 = (OctTreeLeafNode *) cl::sycl::malloc_shared(sizeof(OctTreeLeafNode) * nbodies, gpu_queue);
