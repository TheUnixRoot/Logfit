...
//
//                lf->recordCPUTh((bundle->end - bundle->begin), (end - start).seconds()*1000);
//
//#ifdef DEBUG
...
//#ifdef DEEP_GPU_REPORT
//		char version[100];
//		sprintf(version, "%s_Logfit_deep_GPU_report_%d_%d.txt",pars->benchName, nCPUs, nGPUs);
...
//                        pipeline pipe;
//                        MySerialFilter serial_filter(begin, end, nCPUs, nGPUs, lfe);
...
//		}
//		cerr << "*************************" << endl;
//		cerr << nCPUs << SEP << nGPUs << SEP << runtime << SEP << endl;
//
